...
open class AutoScrollView: UIScrollView {
    ...
    @objc private func keyboardWillShow(_ notification: Notification) {
        ...
        if !contentBounds.contains(firstResponderFrame.origin) {
            let duration = (notification as NSNotification).userInfo?[UIKeyboardAnimationDurationUserInfoKey] as? TimeInterval ?? Constants.DefaultAnimationDuration
            let curve = (notification as NSNotification).userInfo?[UIKeyboardAnimationCurveUserInfoKey] as? UIViewAnimationCurve ?? Constants.DefaultAnimationCurve
